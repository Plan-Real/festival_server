...
exports.Convert = (req, res) => {
    ...
    if (cmd == 'convert') {
        fs.readFile("view/assets/photo/"+selected+".jpg", {encoding:'base64'}, (err, data) => {
            request.post(
                {
                    url: "http://192.168.0.11:8000/convert",
                    body: data,
                    headers: {
...
exports.Print = (req, res) => {
    const cmd = req.params['0']
    console.log("aaaaaaaaaaaaaa",cmd)
    Print.exec(req, res, cmd)
}

exports.Save = (req, res) => {
    var num = req.params['0']
    console.log(req.body['image'].substring(23))
    const imgBuffer = Buffer.from(req.body['image'].substring(23), 'base64')
    fs.writeFileSync("view/assets/photo/"+ num +".jpg", imgBuffer)
    res.status(200).json("hi")
}